...
const builtin_packages = [_]BuiltinPackageSpec{
    ...
    .{ .venom_id = "terminal", .kind = "terminal", .default_provider_scope = "host_local", .default_target_path = "/nodes/local/venoms/terminal", .categories_json = "[\"terminal\",\"exec\"]", .hosts_json = "[\"spiderweb\",\"node\"]", .projection_modes_json = "[\"host_local\",\"workspace_service\",\"node_export\"]", .help_md = "Command execution service with Linux-only interactive terminal sessions." },
