...
- `/nodes/local/venoms/*` is the canonical local-service origin when no workspace-bound `/services/*` path is available yet.
...
14. Discover available services through `/projects/<workspace_id>/meta/mounted_services.json` first, then fall back to `/nodes/local/venoms/VENOMS.json` for the local catalog if needed. `/global/venoms/VENOMS.json` remains the compatibility discovery index.
