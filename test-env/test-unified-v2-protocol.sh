...
if [[ -z "$SPIDERWEB_PORT" ]]; then
    SPIDERWEB_PORT="$(pick_free_port "$BIND_ADDR")"
fi

if [[ -z "$FS_NODE_PORT" ]]; then
    while true; do
        candidate_port="$(pick_free_port "$BIND_ADDR")"
        if [[ "$candidate_port" != "$SPIDERWEB_PORT" ]]; then
            FS_NODE_PORT="$candidate_port"
            break
        fi
    done
fi
...
def run_control_runtime_order(host: str, port: int, auth_token=None) -> None:
    def scenario_control_before_version() -> None:
        conn = WSConn.connect(host, port, "/", auth_token)
        try:
            conn.send_json({
                "channel": "control",
    ...
    def scenario_attach_before_version() -> None:
        conn = WSConn.connect(host, port, "/", auth_token)
        try:
            conn.send_json({
                "channel": "control",
            ...
            require(err.get("code") == "unsupported_transport", f"unexpected acheron error code: {err}")
            err_message = str(err.get("message", ""))
            require(
                "control.mount_*" in err_message or "spiderweb-fs" in err_message,
                f"unexpected acheron error message: {err}",
            )
            opcode, _ = conn.read_frame()
            require(opcode == 0x8, "expected close frame after external-runtime rejection")
        finally:
            conn.close()
    ...
    def scenario_happy_path() -> None:
        ...
        try:
            ...
            expect_type(err_msg, "acheron", "acheron.error")
            err = err_msg.get("error") or {}
            require(err.get("code") == "unsupported_transport", f"unexpected acheron version error: {err}")
            err_message = str(err.get("message", ""))
            require(
                "control.mount_*" in err_message or "spiderweb-fs" in err_message,
...
def run_source_guard(root_dir: str) -> None:
    checks = [
        ...
        ("src/runtime/fs/shared/fs_node_main.zig", 'const node = @import("spiderweb_node").fs_node_main;'),
        ("deps/spider-node/src/spiderweb_node/fs_node_main.zig", '\\"channel\\":\\"control\\",\\"type\\":\\"control.version\\"'),
        ("deps/spider-node/src/spiderweb_node/fs_node_main.zig", '\\"channel\\":\\"control\\",\\"type\\":\\"control.connect\\"'),
        ("deps/spider-node/src/spiderweb_node/fs_node_main.zig", '\\"channel\\":\\"acheron\\",\\"type\\":\\"acheron.t_fs_hello\\"'),
    ]
